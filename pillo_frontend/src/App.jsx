...
import { Camera, Play, Square, Eye, EyeOff, Settings, Wifi } from "lucide-react"
...
function App() {
  ...
  const startCamera = useCallback(() => {
    if (!cameraSupported) {
      const reason = !hasGetUserMedia ? "瀏覽器不支援 getUserMedia。請更換支援的瀏覽器。" : "攝影機功能不可用。"
  ...
  const changeFacingMode = useCallback(
    (mode) => {
      ...
      setCameraError(null)
      setDebugInfo(`正在切換到${mode === "environment" ? "後置" : "前置"}鏡頭...`)
  ...
  const changeCamera = useCallback(
    (deviceId) => {
      ...
      setCameraError(null)
      setDebugInfo(
        `攝影機已切換為: ${videoDevices.find((d) => d.deviceId === deviceId)?.friendlyLabel || "未知攝影機"}`,
      )
  ...
  const enumerateAndSelectCamera = useCallback(async () => {
    ...
    try {
      ...
      // 只在設備列表實際改變時更新狀態
      const currentDeviceIds = videoDevices.map(d => d.deviceId).sort().join(',')
      const newDeviceIds = enhancedDevices.map(d => d.deviceId).sort().join(',')
      
      if (currentDeviceIds !== newDeviceIds) {
        setVideoDevices(enhancedDevices)
        console.log("可用攝影機已更新:", enhancedDevices.map((d) => d.friendlyLabel))
      }
  ...
  }, [detectDevice, hasEnumerateDevices, selectedDeviceId, preferredFacingMode, videoDevices])
    ...
    const initializeApp = async () => {
      ...
      if (!cameraSupported) {
        ...
        if (!hasGetUserMedia) {
          ...
          reason = "攝影機功能需要 HTTPS 連線。在區域網環境下，請使用 HTTPS 或 localhost。"
        } else {
          reason = "攝影機功能不可用。"
        }
  ...
  const getStatusColor = () => {
    ...
    if (connectionStatus === "connecting" || (!cameraReady && cameraStarted)) return "text-yellow-400"
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 text-white p-0 md:p-4">
      <div className="w-full md:max-w-6xl md:mx-auto">
        ...
        <div className="text-center mb-4 md:mb-8">
          ...
          <div className={`flex items-center justify-center gap-2 text-lg ${getStatusColor()}`}>
            <Wifi size={20} />
            {getStatusText()}
          </div>
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-gray-100">偵測算法</h3>
                <Select value={status.algorithm} onValueChange={(val) => changeAlgorithm(val)}>
                  <SelectTrigger className="w-full bg-white/10 border-white/20 text-white">
                    <SelectValue placeholder="選擇偵測算法" />
                  </SelectTrigger>
                  <SelectContent className="bg-gray-800 border-gray-600 text-white">
                    <SelectItem value="algorithm1" className="text-white hover:bg-gray-700 focus:bg-gray-700">算法一 (Otsu 二值化)</SelectItem>
                    <SelectItem value="algorithm2" className="text-white hover:bg-gray-700 focus:bg-gray-700">算法二 (Canny 邊緣偵測)</SelectItem>
                    <SelectItem value="yolo11" className="text-white hover:bg-gray-700 focus:bg-gray-700">算法三 (YOLOv11)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-4">
                ...
                {videoDevices.length > 1 && (
                  <div className="space-y-2">
                    ...
                    <Select value={selectedDeviceId || ""} onValueChange={(val) => changeCamera(val)}>
                      <SelectTrigger className="w-full bg-white/10 border-white/20 text-white">
                        <SelectValue placeholder="選擇攝影機" />
                      </SelectTrigger>
                      <SelectContent className="bg-gray-800 border-gray-600 text-white">
                        {videoDevices.map((device) => (
                          <SelectItem key={device.deviceId} value={device.deviceId} className="text-white hover:bg-gray-700 focus:bg-gray-700">
                ...
                <div className="space-y-2">
                  <Label className="text-gray-200">鏡頭方向</Label>
                  <Select value={preferredFacingMode || "environment"} onValueChange={(val) => changeFacingMode(val)}>
                    <SelectTrigger className="w-full bg-white border-white/20 text-black">
                      <SelectValue placeholder="選擇鏡頭方向" />
                    </SelectTrigger>
                    <SelectContent className="bg-gray-800 border-gray-600 text-black">
                      <SelectItem value="environment" className="text-white hover:bg-gray-700 focus:bg-gray-700">📷 後置鏡頭</SelectItem>
                      <SelectItem value="user" className="text-white hover:bg-gray-700 focus:bg-gray-700">🤳 前置鏡頭</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex flex-col md:flex-row flex-wrap gap-2 md:gap-3">
                  <Button
                    ...
                    className="bg-green-600 hover:bg-green-700 text-white border-green-600 disabled:bg-gray-600 disabled:text-gray-300"
                  >
                    <Play size={18} />
                    開始攝影
                  </Button>
                  <Button
                    ...
                    className="bg-red-600 hover:bg-red-700 text-white border-red-600 disabled:bg-gray-600 disabled:text-gray-300"
                  >
                    <Square size={18} />
                    停止攝影
                  </Button>
                  <Button
                    ...
                    className="bg-purple-600 hover:bg-purple-700 text-white border-purple-600 disabled:bg-gray-600 disabled:text-gray-300"
                  >
                    <Camera size={18} />
                    測試捕獲
                  </Button>
                  <Button
                    ...
                    className={`${status.detection_active ? "bg-orange-600 hover:bg-orange-700 border-orange-600" : "bg-blue-600 hover:bg-blue-700 border-blue-600"} text-white disabled:bg-gray-600 disabled:text-gray-300`}
                  >
                    {status.detection_active ? <EyeOff size={18} /> : <Eye size={18} />}
                  ...
                  <div className="text-sm text-gray-200 space-y-1">
                    ...
                    {(selectedDeviceId || preferredFacingMode) && (
                      <div className="text-blue-300">
                        當前使用：
                        {selectedDeviceId
                          ? videoDevices.find((d) => d.deviceId === selectedDeviceId)?.friendlyLabel
                          : `${preferredFacingMode === "environment" ? "後置" : "前置"}鏡頭`}
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="flex justify-center">
              <div className="relative">
                ...
                {cameraSupported && cameraStarted && (
                  <div className="block">
                    <Webcam
                      key={`${selectedDeviceId || preferredFacingMode || "default"}`}
                      ref={webcamRef}
                      audio={false}
                      width={640}
                      height={480}
                    ...
                    <div className="absolute top-2 left-2 bg-black/50 text-white px-2 py-1 rounded text-sm">
                      {status.detection_active ? "偵測中" : cameraReady ? "攝影中" : "初始化中"}
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      {cameraSupported ? (
                        <p className="text-gray-400">
                          {cameraError ? `錯誤: ${cameraError}` : "正在請求攝影機權限..."}
                        </p>
                      ) : (
                        <div className="text-center">
                          ...
                          {!hasGetUserMedia && (
                            <p className="text-sm text-gray-500">請使用現代瀏覽器（Chrome、Firefox、Safari、Edge）</p>
                          )}
                          {!isSecureContext && !isLocalhost && hasGetUserMedia && (
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div>
                ...
                <p className="text-4xl font-bold text-green-400">{status.count}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-lg text-blue-400">{isProcessing ? "處理中..." : "待機"}</p>
              </div>
              {debugInfo && (
